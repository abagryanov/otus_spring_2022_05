...
class Api {
    ...
    async getComments(bookId) {
        ...
        try {
            const {data} = await this._client("/api/books/" + bookId + "/comments/")
            return data
        } catch (error) {
            console.error(error)
        }
    ...
    async deleteComment(bookId, commentId) {
        ...
        try {
            await this._client("/api/books/" + bookId + "/comments/" + commentId, {
                method: "DELETE"
            })
        } catch (error) {
            console.error(error)
    ...
    async addComment(bookId, comment) {
        ...
        try {
            await this._client("/api/books/comments", {
                method: "POST",
                data: {
                    bookId: bookId,
                    comment: comment
